<template>
  <div class="p-8">
    <div class="flex justify-between items-center mb-6">
      ...
      <button class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg font-semibold transition shadow-md">
        + Nuevo Asesor
      </button>
    </div>
...
// 2. Función para traer clientes (con soporte para filtros y paginación)
const fetchClientes = async (page = 1) => {
  try {
    // Enviamos los filtros como parámetros de consulta
    const response = await axios.get(`/api/clientes?page=${page}`, { 
      params: filters.value 
    });
    
    console.log("Datos recibidos:", response.data);
    
    // CORRECCIÓN: Ahora usamos response.data para acceder a los datos
...
// 4. Función de eliminación lógica (desactivación)
const deleteCliente = async (id) => {
  const result = await Swal.fire({
    title: '¿Confirmar acción?',
    text: "El asesor será desactivado del sistema.",
    icon: 'warning',
  ...
  if (result.isConfirmed) {
    try {
      ...
      Swal.fire('¡Éxito!', 'El asesor ha sido desactivado.', 'success');
      // Refrescamos la lista manteniendo la página actual
