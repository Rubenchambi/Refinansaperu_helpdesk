<template>
  <div class="p-8 bg-slate-50 min-h-screen">
    <div class="mb-8">
      <h1 class="text-3xl font-extrabold text-slate-800">Gestión de Roles</h1>
      <p class="text-slate-500">Administra los roles operativos del sistema.</p>
    </div>
      ...
      <div class="lg:col-span-2">
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          ...
          <div class="p-4 border-t border-slate-100 flex justify-between items-center">
            <button :disabled="!pagination.prev_page_url" @click="changePage(pagination.current_page - 1)" class="px-4 py-2 bg-blue-500 rounded-lg text-sm text-white disabled:opacity-50 transition-all hover:bg-blue-600">Anterior</button>
            <span class="text-sm text-slate-500 font-medium">Pág {{ pagination.current_page }} de {{ pagination.last_page }}</span>
            <button :disabled="!pagination.next_page_url" @click="changePage(pagination.current_page + 1)" class="px-4 py-2 bg-blue-500 rounded-lg text-sm text-white disabled:opacity-50 transition-all hover:bg-blue-600">Siguiente</button>
          </div>
        </div>
      </div>
    </div>
